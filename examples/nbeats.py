...
def eval_predictions(model, val_dataloader):
    actuals = torch.cat([y for _, (y, _) in iter(val_dataloader)])
    predictions = model.predict(val_dataloader).to(actuals.device)
...
def main():
    ...
    fit_model(trainer, net, train_dataloader, val_dataloader)
    mae = eval_predictions(net, val_dataloader)
